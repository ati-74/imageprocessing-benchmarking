...
def BacteriaAnalysis(dataFrame,interval_time,growth_rate_method):
    ...
    result_dict= {'CellId': [],'lable':[],'birth Length':[], 'Last Length':[], 'AverageLength': [], 'AverageOrientation': [], 'AverageVelocity':[], 'LifeHistory':[], 'growth rate':[]}
    ...
    dataFrame ["division_Length"]=''
    dataFrame ["lineage"]=''
    dataFrame ["AverageLength"]=''
    dataFrame ["AverageOrientation"]=''
    ...
    for index, row in dataFrame.iterrows():
           if not dataFrame.iloc[index]["growthRate"]:
               ...
               if dataFrame.iloc[index]["drop"]==True:
                   ...
                   result_dict['lable'].append(dfLifehistory.iloc[[0]]["TrackObjects_Label_50"].values[0])
                   result_dict['birth Length'].append(birthLength)
                   result_dict['Last Length'].append(divisionLength)
