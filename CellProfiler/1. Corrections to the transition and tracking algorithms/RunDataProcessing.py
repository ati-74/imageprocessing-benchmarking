...
from ProcessCellProfilerData import ProcessData
...
def StartProcessing():
   ...
   if CheckVar1.get()==1:
      ...
      ProcessData (input_file,output_directory_path_as_input_folder,float(interval_time.get()),growth_rate_method_value.get(), float(um_per_pixel.get()))
        
      msg = messagebox.showinfo( "Analysis were done", "Analysis were done")
      msg = messagebox.showinfo( "The calculation files were written into "+output_directory_path_as_input_folder,  "The calculation files were written into "+output_directory_path_as_input_folder)
      
   else:
      print("output directory: "+ output_directory_path)
      #start processing csv file
      ProcessData (input_file,output_directory_path,float(interval_time.get()),growth_rate_method_value.get(), float(um_per_pixel.get()))
        
      msg = messagebox.showinfo( "Analysis were done", "Analysis were done")
      msg = messagebox.showinfo( "The calculation files were written into "+output_directory_path,  "The calculation files were written into "+output_directory_path)
...
if __name__=="__main__":
   ...
   growth_rate_method_lable = Label(top, text = "Growth rate method:")
   growth_rate_method_lable.pack( side = LEFT)
   growth_rate_method_lable.place(x = 350,y = 40)
   ...
   browse_file_lable.place(x = 30,y = 100)
   ...
   browse_directory_lable.place(x = 350,y = 100)
   ...
   um_per_pixel_lable = Label(top, text = "1 pixel is equal to: ")
   um_per_pixel_lable.pack( side = LEFT)
   um_per_pixel_lable.place(x = 30,y = 200)
   ...
   start_process = Button(top, text = "Start Processing", command = StartProcessing)
