function [] = DeletaAnalysis(dataset,mode,intervalTime,num_time_steps)

write_directory = strcat('../',dataset,'/',mode,'/post-processing/results/');
load(strcat('../',dataset,'/',mode,'/2. results/Position000000.mat'),'res');

cellNumber=length(res{1}.lineage);
minLife=2;

growth_rate=[];
birthLength=[];
lifeHistory=[];
Average_length = [];
Average_Velocity = [];

first_frame=[];
last_frame=[];

number_of_cells_in_each_timesteps=[];

for i=1:cellNumber
    valid_frames_index = find(res{1}.lineage{i}.frames <=num_time_steps);
    valid_frames = res{1}.lineage{i}.frames(valid_frames_index);
    if isempty(valid_frames)==0
        lifeHistoryValue=length(valid_frames);
        Celllength=res{1}.lineage{i}.length(valid_frames_index);
        birthLengthValue=Celllength(1);
        lastLengthValue=Celllength(end);

        if lifeHistoryValue >= minLife
            growth_rateValue=(log(lastLengthValue)-log(birthLengthValue))/ (lifeHistoryValue * intervalTime);
            %average velocity
            x_center = res{1}.lineage{i}.x_center(valid_frames_index);
            y_center = res{1}.lineage{i}.y_center(valid_frames_index);
            x1=sqrt(x_center(1)^2+y_center(1)^2);
            x2=sqrt(x_center(end)^2+y_center(end)^2);
            AverageVelocityValue=(x2-x1)/(lifeHistoryValue*intervalTime); 
            Average_length(end+1)=mean(Celllength);
        else
            growth_rateValue=NaN;
            AverageVelocityValue = NaN;
            Average_length(end+1)=mean(NaN);
        end

        %first & last timeStep
        firstFrameValue=valid_frames(1);
        lastFrameValue=valid_frames(end);    
        %save Results
        growth_rate(end+1)=growth_rateValue;
        birthLength(end+1)=birthLengthValue;
        lifeHistory(end+1)=lifeHistoryValue;
        Average_Velocity (end+1) = AverageVelocityValue;
        first_frame(end+1)=firstFrameValue;
        last_frame(end+1)=lastFrameValue;
    end
end

Number_of_timesteps=max(last_frame);
cellLifeHistory=transpose([first_frame;last_frame]);
%number of cell per time steps
for j=1:Number_of_timesteps
    number_of_cells=length(cellLifeHistory(j>=cellLifeHistory(:,1) & j<= cellLifeHistory(:,2)));
    number_of_cells_in_each_timesteps(end+1)=number_of_cells;
end  

%add to table
T = table(transpose(1:length(growth_rate)),transpose(growth_rate),transpose(birthLength),transpose(lifeHistory),transpose(Average_Velocity),transpose(Average_length));
T2=table(transpose(1:length(number_of_cells_in_each_timesteps)),transpose(number_of_cells_in_each_timesteps));
%add column name
T.Properties.VariableNames={'CellId','GrowthRate','birthLength','LifeHistory','AverageVelocity','AverageLength'};
T2.Properties.VariableNames={'StepNumber','NumberOfCells'};

% write to csv
writetable(T,strcat(write_directory,'DeLTA_LifeHistory_based_Analysis.csv'),'Delimiter',',','QuoteStrings',true)
writetable(T2,strcat(write_directory,'DeLTA_Num_cells_in_each_timeStep.csv'),'Delimiter',',','QuoteStrings',true)


