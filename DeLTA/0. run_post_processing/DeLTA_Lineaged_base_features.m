function lable = DeLTA_Lineaged_base_features(dataset,mode,num_time_steps)
...
for element_indx = 1 : length(unique_lable)
    ...
    if num_division_val >1
        num_division (end + 1) = num_division_val;
    else
        num_division (end + 1) = NaN;
    end
...
% write to csv
writetable(T2,strcat(write_directory,'DeLTA_lineage_based_analysis.csv'),'Delimiter',',','QuoteStrings',true)
