...
def assign_parent_daughter(df):
    ...
    @param df DataFrame containing cell data with columns "CellLifeId" and "parent".

    Returns:
    - df DataFrame The modified DataFrame with the 'daughters' column added, containing a list of daughter CellLifeIds
    ...
    for index, row in df.iterrows():
        ...
        if not df.iloc[index]['checked']:
            # Find all entries (rows) for the same cell (identified by "CellLifeId").
            same_bac = df.loc[df["CellLifeId"] == df.iloc[index]["CellLifeId"]]
            same_bac_index = same_bac.index.values.tolist()

            # Identify potential daughter cells based on the 'parent' column.
            daughters = df.loc[df['parent'] == df.iloc[index]["CellLifeId"]].reset_index(drop=True)

            # If the cell has one or no daughters, set daughters to 0.
            # Otherwise, set 'daughters' to a list of unique daughter "CellLifeId" values.
            if daughters.shape[0] <= 1:
                daughters = 0
            else:
                daughters = list(set(daughters["CellLifeId"].values.tolist()))
...
def bacteria_analysis(data_frame, interval_time):
    """
    ...
    for index, row in data_frame.iterrows():
        ...
        if row['ImageNumber'] < last_time_step:
            next_time_step_bacteria = data_frame.loc[(data_frame['TrackObjects_ParentImageNumber_50'] == row['ImageNumber'])
                                                     & (data_frame['TrackObjects_ParentObjectNumber_50'] == row['ObjectNumber'])].reset_index(drop=True)
        ...
        if not data_frame.iloc[index]["growthRate"] and data_frame.iloc[index]["growthRate"] != 0:
            ...
            if parent_obj_number == 0:
                ...
                parent = parent["CellLifeId"].values[0]
            for idx in life_history_index:
                data_frame.at[idx, "CellLifeId"] = id_of_bacteria
                data_frame.at[idx, "growthRate"] = elongation_rate
                data_frame.at[idx, "parent"] = parent
    ...
    final_data_frame = final_data_frame[["TimeStep", "CellLifeId", "Orientation", "label", "Center_X", "Center_Y",
                                         "InstantaneousGrowthRate", "InstantaneousVelocity", "divideFlag", "Major_axis",
                                         "Minor_axis", "parent"]]
