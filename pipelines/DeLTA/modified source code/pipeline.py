...
class Pipeline:
    ...
    def process(
        ...
        if features is None:
            features_list = ["length", "width", "area", "perimeter", "edges", "x_center", "y_center",
...
class Position:
    ...
    def features(
        ...
        features : list or tuple of str, optional
            ...
            The default is ("length", "width", "area", "perimeter", "edges", "x_center", "y_center",
                                     "orientation", "rectangle_width", "rectangle_height").
    ...
    def legacysave(self, res_file: Union[str, Path]) -> None:
        ...
        for r in self.rois:
            ...
            for c in cells:
                ...
                if "x_center" in c:
                    lin[-1]["x_center"] = np.array(c["x_center"], dtype=np.float32)
                if "y_center" in c:
                    lin[-1]["y_center"] = np.array(c["y_center"], dtype=np.float32)
                if "orientation" in c:
                    lin[-1]["orientation"] = np.array(c["orientation"], dtype=np.float32)
                if "rectangle_width" in c:
                    lin[-1]["rectangle_width"] = np.array(c["rectangle_width"], dtype=np.float32)
                if "rectangle_height" in c:
                    lin[-1]["rectangle_height"] = np.array(c["rectangle_height"], dtype=np.float32)
